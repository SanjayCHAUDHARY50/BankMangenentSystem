...
def back():
    ...
    if con == "Y":
     ...
     w = input("Warning your recovery data may loss, Are confirm to exit? Y/n :")
     if w == "Y":
         exit(0)
     else:
         back()
...
def update_date():
    ...
    print("\n1: Card Number \n2: Name\n3: Age\n4: NRIC\n5: Amount (RM)")
    memory = {"1": "Card Number", "2": "Name","3": "Age ","4": "NRIC", "5" :"Amount (RM)"}
    k = input()
...
def delete_data():
    ...
    if danger == "1":
        ...
        if confirm == "Y":
            ...
            with open("User_data.csv", "w", newline='') as file:
                csv_writer = csv.DictWriter(file, fieldnames=["Card Number", "Name", "Age", "NRIC", "Amount (RM)",
                                                              "Registered Date"])
                file.seek(0, 2)
                if file.tell() == 0:
                    csv_writer.writeheader()  # Write only Once
